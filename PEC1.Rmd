...
## 3.1 Estudio de la distribución
...
Para obtener el resultado de test de Levene para todas las réplicas, primero crearemos un nuevo dataframe vacío que contendrá el resultado del test. Seguidamente, iteraremos las muestras en que el sufijo "_MSS" o "_PD" indicará el grupo, y el sufijo "_1" o "_2" indicará la réplica. 

Entonces, ejecutaremos un bucle `if else`, en que se verifica si existe una columna MSS (if, col1 y col2) o PD (else, col3 o col4) y comparará los datos de la varianza de la réplica 1 (_1) respecto a la réplica 2 (_2)

```{r}
# Obtener los nombres de las columnas para las réplicas
...
for (muestra in muestras) {
  # Definir los nombres de las columnas de las réplicas MSS
  col1 <- paste0(muestra, "_1_MSS")
  col2 <- paste0(muestra, "_2_MSS")
  
  # Definir los nombres de las columnas de las réplicas PD
  col3 <- paste0(muestra, "_1_PD")
  col4 <- paste0(muestra, "_2_PD")
  
  # Verificar si ambas columnas MSS existen
  if (col1 %in% colnames(abundance_data) & col2 %in% colnames(abundance_data)) {
    # Extraer las abundancias de las dos réplicas MSS
    abundancias <- abundance_data[, c(col1, col2)]
    # Crear un dataframe para el test de Levene
    datos <- data.frame(
    ...
    #En caso de no ser MSS, será PD, y repetiremos el proceso para el resto de muestras PD
...
Después, dividiremos los dos grupos tumorales (MSS y PD).
...
Finalmente, crearemos un gráfico con los dos grupos 
...
Vemos que, de entre los dos grupos tumorales, parece ser que el grupo PD tiene una abundancia superior (de hecho, es bastante similar, pero parece que el PD tiene 3 valores mucho más altos). 
...
Como ya tenemos los datos de la muestra en `abundance_long`, solo deemos crear el boxplot en base a esos datos, con las muestras (MSS y PD) en base a su abundancia. Además, como tenemos 12 muestras, inclinamos las etiquetas del eje X para facilitar su lectura.
...
```
Vemos que, en el caso del grupo MSS, se ve mucho más representad por la componente principal 2, mientras que el grupo PD se ve mucho más representado por la componente principal 1. Aun así, debemos tener en cuenta que cada componente principal representa cierta cantidad de la variabilidad de los datos. 
...
```
Después del test anova en todos los fosfopéptidos vemos que, de los 1438 fosfopéptidos analizados, solo 104 muestran diferencias significativas entre los grupos MSS y PD. 
